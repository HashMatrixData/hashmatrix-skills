...
### Sub-task 清单
#### 1. [标题]
- 目标 / 范围(文件清单) / 起点(精确路径 + 真·stub·缺失) / 预估 LOC / 测试 / 依赖(blocked-by·blocks) / 可独立 ship(是·否+原因) / 验收
#### 2. ...
### 依赖图  [ASCII]
...
凭行数硬切破 main → 用「main 可消费」验证每刀｜Sub-task 层硬套「main 可消费」过度打包 → 用 0.5 层级化约束｜为小 PR 引入 Optional default/兼容层 → 承认必要打包｜不同职责塞同一 sub-task → 每个一句话说清职责｜漏集成回归 → 末端留集成测试｜切刀前不画依赖图 → 先画 blocks｜跳过用户确认直接建子项 → 方案确认后才下单｜一个 Sub-Issue 跨多仓 / 把跨仓工作硬塞进单仓子项 → 一仓一子项，跨仓拆分各自建于对应仓、用文本回链串联｜子任务标题/正文含客户可识别信息 → 提报前按红线脱敏自检（grep）。
